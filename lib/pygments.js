...
function highlight (source, language, highlightedCb) {
    var pygments = spawn(pygmentize, ['-l', language.name, '-f', 'html', '-O', 'encoding=utf-8,full=false'])  
      , output   = ''
      ;
...
function copyStyles(sourceDir, targetDir, copiedStylesCb) {
    ...
    mkdirp(targetDir, function (err) {
        ...
        , function (err, res) {
                ...
                console.log('copying %s files', pumping);
                ...
                res.files.forEach(function (fi) {
                    ...
                    util.pump(source, target, function (err) {
                        if (err) console.log('done pumping with error: ', err);
