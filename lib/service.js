...
module.exports.convertFolder = function (rootPath, opts, convertedFolderCb) {
    ...
    function bailedOnError (desc, err) {
        return didBailOnError(desc, err, convertedFolderCb);
    }
    ...
    function init() {
        ...
        this.data.stylesTargetPath = path.join(this.data.targetPath, common.StylesFolderName);
    ...
    function prepareTargetDirectories() {
        mirrorDirectories(this.data.targetPath, this.data.entryInfos.directories, this);
    }
    ...
    function syntaxHighlightFiles () {
        ...
        function getReaderCodePage(htmlFragment, fi) {

            var relStylesPath = path.relative(fi.fullParentDir, that.stylesTargetPath);
            ...
            if (isMarkdown(fi.name)) {
                ...
                if (opts.highlighter === 'pygment') {
                    ...
                    pygments.highlightFile(fi.fullPath, function (err, htmlFragment) {
                        ...
                        var pageHtml = getReaderCodePage(htmlFragment, fi);
...
module.exports.convertFolder(root, { directoryFilter: ['!node_modules', '!.git'] }, function () {
    console.log('highlighted', arguments);
});
