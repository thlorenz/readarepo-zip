...
// API 
function cloneGitRepository(url, opts, clonedGitRepoCb) {
    opts =  opts || { };
    function init() {
        this.data = {
            url        :  url 
    ...
    function prepareTargetDirectory() {
        this.data.cloneRepoPath = path.join(this.data.fullTargetPath, this.data.repoName);
        mkdirp(this.data.fullTargetPath, this);
    }
    
    function cloneRepo() {
        sh.gitClone(this.data.url, this.data.cloneRepoPath, this);
    }
    ...
    step(init
       ...
       , function cloned(err) { 
           ...
           clonedGitRepoCb(this.data.cloneRepoPath);
         }
       );
}

cloneGitRepository('git://github.com/thlorenz/readarepo.git', null, function(path) {
   console.log('repoed', path); 
});
